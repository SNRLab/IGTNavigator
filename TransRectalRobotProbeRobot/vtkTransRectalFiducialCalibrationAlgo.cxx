...
//--------------------------------------------------------------------------------------
bool vtkTransRectalFiducialCalibrationAlgo::CalibrateFromImage(const TRRobotProbeBiopsyCalibrationFromImageInput &input)
{ 
...
struct PointTooFar 
{
  ...
  bool operator()( itk::Point<double, 3> x) {
    return RobotProbeNavMath::PointDistanceFromLineM(x,defp_p,defv_p)>max_distance;
  }
};
...
void vtkTransRectalFiducialCalibrationAlgo::RemoveOutliners(double P_[3], double v_[3], const double def1[3], const double def2[3], std::vector<PointType> &CoordinatesVector)
{
    ...
    do {
        ...
        for (i=CoordVectorCopy.begin(); i!=CoordVectorCopy.end(); ++i){
            dist=RobotProbeNavMath::PointDistanceFromLineM(*i,P_,v_);
...
bool vtkTransRectalFiducialCalibrationAlgo::FindTargetingParams(vtkRobotProbeNavTargetDescriptor *target, const TRRobotProbeBiopsyCalibrationData &calibrationData, NeedleDescriptorStruct *needle, TRRobotProbeBiopsyTargetingParams *targetingParams)
{
  if (target==NULL)
  {
    return false;
