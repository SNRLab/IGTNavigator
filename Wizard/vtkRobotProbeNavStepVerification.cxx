...
//----------------------------------------------------------------------------
vtkStandardNewMacro(vtkRobotProbeNavStepVerification);
vtkCxxRevisionMacro(vtkRobotProbeNavStepVerification, "$Revision: 1.1 $");

//----------------------------------------------------------------------------
vtkRobotProbeNavStepVerification::vtkRobotProbeNavStepVerification()
{
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::SetVerificationPointListNode(vtkMRMLFiducialListNode *node)
{
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::ShowUserInterface()
{
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::ShowVolumeSelectionFrame()
{
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::ShowTargetListFrame()
{
  vtkKWWidget *parent = this->GetGUI()->GetWizardWidget()->GetClientArea();
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::ShowVerificationControlFrame()
{
  vtkKWWidget *parent = this->GetGUI()->GetWizardWidget()->GetClientArea();
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::ProcessGUIEvents(vtkObject *caller,
                                          unsigned long event, void *callData)
{
  ...
  {    
    ...
    vtkRobotProbeNavTargetDescriptor* targetDesc=NULL;
    if (this->TargetIndexUnderVerification>=0)
    {
      targetDesc=this->GetRobotProbeNavManager()->GetTargetDescriptorAtIndex(this->TargetIndexUnderVerification);      
    }
    else
    {
      targetDesc=this->GetRobotProbeNavManager()->GetTargetDescriptorAtIndex(this->GetRobotProbeNavManager()->GetCurrentTargetIndex());
    }
    if (targetDesc!=NULL)
    {
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::ProcessMRMLEvents(vtkObject *caller,
                                         unsigned long event, void *callData)
{  
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::AddMRMLObservers()
{
  if (this->VerificationPointListNode!=NULL)
  {
    vtkSmartPointer<vtkIntArray> events = vtkSmartPointer<vtkIntArray>::New();
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::RemoveMRMLObservers()
{
  vtkMRMLRobotProbeNavManagerNode* manager=this->GetRobotProbeNavManager();
  if (manager!=NULL)
  {
...
//---------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::OnMultiColumnListSelection()
{
  ...
  if (numRows == 1)
    ...
    this->GUI->BringTargetToViewIn2DViews(vtkRobotProbeNavGUI::BRING_MARKERS_TO_VIEW_ALIGN_TO_NEEDLE);
    }
}

//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::UpdateTargetListGUI()
{
    ...
    if (target->GetTargetValidated())
      {
        ...
        {
          std::ostrstream os;    
          os << std::setiosflags(ios::fixed | ios::showpoint) << std::setprecision(vtkRobotProbeNavGUI::POSITION_PRECISION_DIGITS*2); // *2 to have double precision as the position that it was computed from
        ...
        {
        std::ostrstream os;    
        os << std::setiosflags(ios::fixed | ios::showpoint) << std::setprecision(vtkRobotProbeNavGUI::POSITION_PRECISION_DIGITS*3);
...
//-----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::AddGUIObservers()
{
...
//-----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::RemoveGUIObservers()
{
  if (this->LoadVerificationVolumeButton)
    {
...
//--------------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::UpdateGUI()
{
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::HideUserInterface()
{
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::TearDownGUI()
{  
  RemoveGUIObservers();
  RemoveMRMLObservers();
...
//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::StartVerification()
{
  ...
  if (this->GetRobotProbeNavManager()->GetCurrentTargetIndex()<0)
  {
    // no selected target
    return;
  }        
  ...
  this->TargetIndexUnderVerification=this->GetRobotProbeNavManager()->GetCurrentTargetIndex();
}

//----------------------------------------------------------------------------
void vtkRobotProbeNavStepVerification::StopVerification()
{
...
void vtkRobotProbeNavStepVerification::UpdateVerificationResultsForCurrentTarget()
{
  if (this->TargetIndexUnderVerification<0)
  {
  ...
  vtkRobotProbeNavTargetDescriptor* targetDesc=this->GetRobotProbeNavManager()->GetTargetDescriptorAtIndex(this->TargetIndexUnderVerification);
  ...
  overallErr = RobotProbeNavMath::ComputeDistanceLinePoint(targetDesc->GetNeedleTipValidationPosition(), targetDesc->GetNeedleBaseValidationPosition(),
  ...
  this->GetRobotProbeNavManager()->Modified();
}

void vtkRobotProbeNavStepVerification::DisplayVerificationResultsForCurrentTarget()
{
  if (this->TargetIndexUnderVerification>=0)
  {
  ...
  if (GetRobotProbeNavManager()==NULL)
  {
    vtkErrorMacro("Invalid manager");
    return;
  }

  int targetIndex=GetRobotProbeNavManager()->GetCurrentTargetIndex(); 
  vtkRobotProbeNavTargetDescriptor* targetDesc=NULL;
  if (targetIndex>=0)
  {
    targetDesc=this->GetRobotProbeNavManager()->GetTargetDescriptorAtIndex(targetIndex);    
  }
